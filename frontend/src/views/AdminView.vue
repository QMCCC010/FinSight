...
<template>
  <div class="page-wrap">
    ...
    <section class="section-card"><div class="section-title"><div><h2>RAG索引</h2><p>Milvus执行中文BM25与BGE稠密向量混合检索；连接异常时可降级到本地FAISS/BM25。</p></div><div class="button-group"><button class="secondary-button" @click="reprocessBatch(true)">重处理失败资料</button><button class="secondary-button" @click="reprocessBatch(false)">重处理最近100篇</button></div></div><div class="index-health"><span :class="indexStatus?.compatible ? 'health-ok' : 'health-warning'">{{ indexStatus?.compatible ? '索引兼容' : '需要重建' }}</span><span>后端 {{ indexStatus?.backend || '未知' }}</span><span v-if="indexStatus?.synchronized !== undefined" :class="indexStatus.synchronized ? 'health-ok' : 'health-warning'">{{ indexStatus.synchronized ? '数据已同步' : '数据同步中' }}</span><span>向量数 {{ indexStatus?.metadata?.actual_count ?? 0 }} / 切片 {{ indexStatus?.metadata?.database_count ?? '-' }}</span><span>维度 {{ indexStatus?.metadata?.actual_dimension ?? '-' }}</span><span>模型 {{ indexStatus?.metadata?.embedding_model || '未记录' }}</span><span v-if="indexStatus?.error" class="source-error">异常：{{ indexStatus.error }}</span></div></section>
